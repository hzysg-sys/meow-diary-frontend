...
- `src/components/`
  ...
  - `ReadTab.jsx` — 阅读壳子：书架、书签、背景色、划线菜单、批注讨论、M4 阅读循环（15 秒停留确认已读页 → 批量 `read-events` 上报 + 前瞻窗口、4 秒轮询划线增量、关书触发记忆压缩、恢复位置取"最新书签 vs 关书位置"更晚者）。文件较大，App.jsx 里做了 lazy 按需加载
  - `reader/PagedReader.jsx` — 自研无 iframe 阅读内核：CSS 分栏 + transform 翻页、mk1 锚点（章/段/字符偏移，`getPageData`/`getLookahead` 供 M4 用）、划线 span + 正文流批注条、跨页选择（选区贴边自动翻页）、重排锚点保位
  - `reader/epubLoader.js` — epub.js 仅作解析器：解压/OPF/目录/图片 blob，章节 HTML 缓存
